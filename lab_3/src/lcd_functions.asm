...
lcd_display_input:
	do_lcd_data_from_register		input
	ret
...
LCD_display_result:
    ...
    ;tst     result                     ; Test if result is negative (MSB set)
	mov		r17,			result


	check_display_mode:
		clr		r29						; clear non leading zero flag
		cpi     display_mode, 0xFF
		breq    display_hex

	display_decimal:
		sbrc	result, 7
		rcall    negative                   ; If the number is negative, branch to negative
		;do_lcd_data_from_immediate 'H'
		ldi		r16,			100
		rcall   extract_digit			; Call function to extract and display digits (by power of 10)
		;do_lcd_data_from_immediate 'M'
		ldi		r16,			10
		rcall   extract_digit			; Call function to extract and display digits (by power of 10)
		;do_lcd_data_from_immediate 'L'
		ldi		r16,			1
		rcall   extract_digit			; Call function to extract and display digits (by power of 10)
		rjmp    display_done
	...
	display_hex:
		...
		do_lcd_data_from_immediate '0'
		do_lcd_data_from_immediate 'x'
		sbrc	result, 7
		rcall    negative                   ; If the number is negative, branch to negative
		ldi		r16,			16
		rcall   extract_digit			; Call function to extract and display digits (by power of 16)
		ldi		r16,			1
		rcall   extract_digit			; Call function to extract and display digits (by power of 10)
		rjmp    display_done
	...
	display_done:
		...
		sbrc		overflow_flag, 0
		rcall		led_flash

		ret

	negative:
		do_lcd_data_from_immediate '-'     ; Display the negative sign
		com     r17                      ; Take 1's complement of the number
		inc     r17                      ; Add 1 to get the absolute value (2's complement)
		out		PINC,		r17
		ret           
